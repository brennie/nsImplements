...
template <typename T> struct TearOff {};
...
constexpr nsISupports *InterfaceCast(void *that) {
  return reinterpret_cast<nsISupports *>(
      ...
      InterfacePointerOffset<Derived, Interface>());
}

template <typename Derived, typename... Ts> struct QueryInterfaceImpl;
...
#ifdef DEBUG
    std::cerr << "Comparing T::INTERFACE_ID (" << T::INTERFACE_ID
              << ") vs desired " << uuid << std::endl;
#endif
...
template <typename Derived> struct QueryInterfaceImpl<Derived> {
...
template <typename... Bases> class nsImplements_Bases;

template <> class nsImplements_Bases<> {};

template <typename Base, typename... Bases>
class nsImplements_Bases<implements_traits::TearOff<Base>, Bases...>
    : public nsImplements_Bases<Bases...> {
  static_assert(std::is_base_of_v<nsISupports, Base>,
                "base does not inherit from nsISupports");
};

static_assert(!std::is_base_of_v<
              nsISupports,
              nsImplements_Bases<implements_traits::TearOff<nsISupports>>>);

template <typename Base, typename... Bases>
class nsImplements_Bases<implements_traits::Conditional<Base>, Bases...>
    : public Base, public nsImplements_Bases<Bases...> {
  static_assert(std::is_base_of_v<nsISupports, Base>,
                "base does not inherit from nsISupports");
};
...
static_assert(std::is_base_of_v<
              nsISupports,
              nsImplements_Bases<implements_traits::Conditional<nsISupports>>>);
